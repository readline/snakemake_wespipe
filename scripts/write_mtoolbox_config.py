...
def main():
    try:
        ...
        sys.exit(sys.argv[0] + ' [chrM bam] [Output path] [Sample]')
...
minqual=25
##"""
    newconfig = config.replace('INPATHINPATH','%s/%s'%(os.getcwd(),os.path.dirname(chrmbam))).replace('OUTPATHOUTPATH', '%s/%s'%(os.getcwd(),outpath)).replace('SAMPLESAMPLE',sample)
    print(newconfig)
